...
const Chat = () => {
  ...
  const scrollToBottom = () => {
    console.log(messagesScrollRef.current); // This should print the DOM element
    setTimeout(() => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        "https://admin-backend-2-xhbphobypa-uc.a.run.app/chain",
  ...
  const fetchSummary = async () => {
    try {
      const response = await axios.post(
        "https://admin-backend-2-xhbphobypa-uc.a.run.app/assistant",
        {
  ...
  const handleButtonClick = async (title) => {
    try {
      ...
      setMessages((prevMessages) => {
        ...
        setTimeout(scrollToBottom, 0); // this will be executed after the state is updated
  ...
  useEffect(() => {
    // small delay to ensure that the DOM has updated
    scrollToBottom();
  }, [messages]);
  return (
